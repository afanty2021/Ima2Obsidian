...
> 一次仍脱落即触发自愈——重启 ima + 重新导航到目标库 + 刷新句柄重解析当前页续走
> （已完成卡片靠 processed_titles 去重跳过，弃卡自动补试；自愈页不滚动）。
> 单库自愈上限 `MAX_WEDGE_RESTARTS_PER_KB`=5，耗尽才中止本库（已提取文章照常进
> 保存阶段）。运行背景：macOS 27 的 AX 状态随开机时长退化（约 2 天起明显），重启
> 机器即清零；ima 已设置 NSAppSleepDisabled=1 但不能完全防脱落。
